...
<meta charset="UTF-8">
    ...
    <!-- Link Css -->
    <link rel="stylesheet" href="./css/data_pelanggan.css">
                    ...
                    <tbody>
                        <?php
                            ...
                            if (isset($_POST["cari"])) {
                                ...
                                $cari=$_POST['cari'];
                                $query_pelanggan= mysqli_query($conn,"SELECT * FROM pembeli WHERE id_pembeli LIKE '%$cari%' OR nama_pembeli LIKE '%$cari%' OR username LIKE '%$cari%' OR alamat LIKE '%$cari%'");
                            }else{
                                //jika tidak ada keyword pencarian
                                $query_pelanggan= mysqli_query($conn,"SELECT * FROM pembeli");
                            }
                            while($data_pembeli= mysqli_fetch_assoc($query_pelanggan)) { ?>
                                <tr>
                                    <td><?= $data_pembeli["nama_pembeli"]; ?></td>
                                    <td><?= $data_pembeli["alamat"]; ?></td>
                                    <td><?= $data_pembeli["no_telp"]; ?></td>
                                    <td><?= $data_pembeli["username"]; ?></td>
                                    <td> <a href="hapus_pelanggan.php?id_pembeli=<?=$data_pembeli['id_pembeli']?>" onclick="return confirm ('Apakah anda yakin menghapus data ini?')" ><i class="fa-solid fa-trash"></i></a></td>
                                </tr>
                            <?php
                            }
                            ?>
